...
const AppPage = () => {
  ...
  useEffect(() => {
    ...
    const nextSelected = notes.find((note) => note.id === selectedId)
    if (!nextSelected) {
      return
    }
    lastSelectedIdRef.current = selectedId

    const nextTitle = nextSelected.title
    const nextContentJson = (nextSelected.content_json ?? createDefaultContent()) as Record<
      string,
      unknown
    >
    const nextContentText = nextSelected.content_text ?? ''
  ...
  const handleCreateNote = async () => {
    ...
    try {
      ...
      lastSelectedIdRef.current = newNote.id
